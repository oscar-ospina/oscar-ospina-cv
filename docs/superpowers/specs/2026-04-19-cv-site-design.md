...
4. Deploy to Vercel Hobby at `oscarospina.com`.
...
- **Client components** only where needed: locale toggle, theme toggle. The scroll transform is **pure CSS** (`animation-timeline: scroll()`), no client JS.
...
Each component stays small and focused (target: well under ~200 lines). Server by default; `"use client"` only on the two toggles.
...
- **`robots.ts`**: allow all.
- **JSON-LD `Person`** schema injected into `[lang]/page.tsx` via a `<script type="application/ld+json">` tag: `name`, `jobTitle`, `url`, `email`, `telephone`, `sameAs: [linkedin, github, stackoverflow]`, `address` (Medellín, CO), `knowsAbout` (top skills).
...
6. **Page assembly:** `[lang]/page.tsx` wires everything, JSON-LD injected.
7. **SEO + metadata:** `generateMetadata`, OG image route, favicon, sitemap, robots.
8. **Analytics + CI + deploy:** Vercel Analytics, GitHub Actions workflow, Vercel project setup, domain configuration.
